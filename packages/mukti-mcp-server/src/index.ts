...
import { ExplorationPath, QueryType, ResourceLink, SocraticTechnique } from './types.js';
...
class SocraticEngine {
  static generateQuestions(
    ...
    const contextualQuestions = this.generateContextualQuestions(query, technique);

    return [...baseQuestions, ...contextualQuestions];
  }

  static getBaseQuestions(queryType: QueryType, technique: SocraticTechnique): string[] {
    const questionMap = {
      [QueryType.CODING]: {
  ...
  static generateContextualQuestions(query: string, technique: SocraticTechnique): string[] {
    ...
    if (query.toLowerCase().includes('error') || query.toLowerCase().includes('bug')) {
      contextualQuestions.push('What was the system doing just before this issue occurred?');
      contextualQuestions.push('What changed recently that might have caused this?');
    }

    if (query.toLowerCase().includes('choose') || query.toLowerCase().includes('decide')) {
      contextualQuestions.push('What criteria are most important for this decision?');
      contextualQuestions.push('What would happen if you delayed this decision?');
    }
  ...
  static generateHints(query: string, queryType: QueryType): string[] {
    ...
    switch (queryType) {
      case QueryType.CODING:
        hints.push('Consider breaking the problem into smaller, testable pieces');
    ...
    switch (queryType) {
      case QueryType.CODING:
        ...
        resources.push({
          ...
          description: 'Systematic approaches to breaking down programming problems',
      ...
      case QueryType.CREATIVE:
        resources.push({
          ...
          description: 'Design thinking methodology for creative problem solving',
          category: 'creativity',
        });
        break;
      case QueryType.DECISION_MAKING:
        resources.push({
          ...
          description: 'Mental models and frameworks for better decision making',
    ...
    switch (queryType) {
      case QueryType.CODING:
        paths.push({
          ...
          description: 'Learn to debug by understanding the problem systematically',
          questions: [
...
server.setRequestHandler(ListResourcesRequestSchema, async () => {
  ...
  for (const technique of Object.values(SocraticTechnique)) {
    resources.push({
      ...
      mimeType: 'text/plain',
      name: `Socratic Technique: ${technique.charAt(0).toUpperCase() + technique.slice(1)}`,
    ...
    tools: [
      {
        ...
        description: 'Guide users through Socratic questioning based on their query',
        inputSchema: {
          type: 'object',
          properties: {
            query: {
            ...
            queryType: {
              ...
              description: 'The type of query (coding, creative, decision_making, etc.)',
            },
            technique: {
              type: 'string',
      ...
      },
      {
        ...
        description: "Explain the Socratic approach and why direct answers aren't provided",
        inputSchema: {
          type: 'object',
          properties: {
            context: {
              type: 'string',
              description: 'Optional context about what the user is trying to understand',
            },
          },
        },
      },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  switch (request.params.name) {
    case 'socratic_inquiry': {
      ...
      const questions = SocraticEngine.generateQuestions(query, queryType, technique);
    ...
    case 'explore_paths': {
      ...
      let response = `🗺️ **Exploration Paths for ${queryType.replace('_', ' ')}**\n\n`;
  ...
  return {
    prompts: [
      ...
      },
      {
        ...
        description: 'Use dialectical questioning to explore opposing viewpoints and tensions',
      },
      {
        name: 'socratic_maieutics',
        description: "Apply maieutic questioning to help 'birth' ideas from existing knowledge",
      },
      {
        name: 'guided_reflection',
        description: 'Provide a framework for systematic self-reflection and inquiry',
      },
    ],
  };
});
