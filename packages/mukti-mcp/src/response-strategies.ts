...
export interface QueryContext {
  ...
  emotionalState: 'frustrated' | 'curious' | 'confident' | 'confused' | 'neutral';
...
export class ResponseStrategyFactory {
  ...
  static generateStrategicResponse(
    ...
    const strategy = this.selectResponseStrategy(queryType, context, preferredTechnique);

    return {
      questions: this.generateStrategicQuestions(query, queryType, strategy, context),
  ...
  private static analyzeQueryContext(query: string): QueryContext {
    ...
    if (complexityIndicators.some((indicator) => lowerQuery.includes(indicator))) {
      complexity = 'high';
    } else if (simpleIndicators.some((indicator) => lowerQuery.includes(indicator))) {
      complexity = 'low';
    }
  ...
  private static selectResponseStrategy(
    ...
    const baseStrategies = {
      [QueryType.CODING]: {
        ...
        secondaryTechniques: [SocraticTechnique.DIALECTIC, SocraticTechnique.COUNTERFACTUAL],
        contextualHints: [
      ...
      [QueryType.CREATIVE]: {
        ...
        secondaryTechniques: [SocraticTechnique.ANALOGICAL, SocraticTechnique.COUNTERFACTUAL],
        contextualHints: [
      ...
      [QueryType.DECISION_MAKING]: {
        ...
        secondaryTechniques: [SocraticTechnique.ELENCHUS, SocraticTechnique.COUNTERFACTUAL],
        contextualHints: [
      ...
      [QueryType.PERSONAL_GROWTH]: {
        primaryTechnique: SocraticTechnique.MAIEUTICS,
        secondaryTechniques: [SocraticTechnique.ELENCHUS, SocraticTechnique.ANALOGICAL],
        contextualHints: [
      ...
      [QueryType.LEARNING]: {
        primaryTechnique: SocraticTechnique.MAIEUTICS,
        secondaryTechniques: [SocraticTechnique.DIALECTIC, SocraticTechnique.ANALOGICAL],
        contextualHints: [
      ...
      [QueryType.PROBLEM_SOLVING]: {
        ...
        avoidancePatterns: ['The solution is...', 'You should try...', "Here's how to fix it..."],
        encouragementPhrases: [
  ...
  private static generateStrategicQuestions(
    ...
    if (context.stuckIndicators.includes('technical_issue')) {
      questions.push('What was the last thing that worked before this issue appeared?');
      questions.push('What would happen if you started with a minimal working example?');
    }
    ...
    if (context.priorKnowledge === 'beginner') {
      questions.push('What foundational concepts might be important to understand first?');
  ...
  private static generateContextualHints(
    ...
    if (context.emotionalState === 'frustrated') {
      ...
      hints.push("Sometimes the best insights come when we're not forcing them");
    }
    ...
    switch (queryType) {
      case QueryType.CODING:
        resources.push({
          ...
          description: 'A method of debugging code by explaining it line-by-line',
          category: 'debugging',
        });
   ...
   */
  private static generateNextSteps(queryType: QueryType, context: QueryContext): string[] {
    const steps = [];
   ...
   */
  private static generateAlternativePaths(queryType: QueryType, context: QueryContext): string[] {
    const paths = [];
   ...
   */
  private static generateEncouragement(context: QueryContext, strategy: ResponseStrategy): string {
